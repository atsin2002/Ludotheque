...
if(isset($_POST['payer']))
{
	for($i=0 ;$i < count($_SESSION['panier']['id_produit']) ; $i++) 
	{
		...
		if($produit['stock'] < $_SESSION['panier']['quantite'][$i])
		{
			...
			$contenu .= '<div class="erreur">Quantitè demandèe: ' . $_SESSION['panier']['quantite'][$i] . '</div>';
			if($produit['stock'] > 0)
			{
				$contenu .= '<div class="erreur">la quantitè de l\'produit ' . $_SESSION['panier']['id_produit'][$i] . ' A été réduite car notre stock était insuffisant, veuillez vérifier vos achats.</div>';
	...
	if(!isset($erreur))
	{
		...
		unset($_SESSION['panier']);
		mail($_SESSION['membre']['email'], "confirmation de la commande", "Merci votre numero de suivi est le $id_commande", "From:vendeur@dp_site.com");
